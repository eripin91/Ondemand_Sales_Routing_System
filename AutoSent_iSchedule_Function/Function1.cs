...
namespace AutoSent_iSchedule_Function
{
    public static class Function1
    {
        ...
        [FunctionName("AutoSentMessage")]
        public static async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", "post", Route = "AutoSentMessage/{createdon?}")] HttpRequest req,
            ILogger log)
        {
            try
            {
                ...
                DateTime localNow = (createdon == "" || createdon==null) ? DateTime.UtcNow : Convert.ToDateTime(createdon);
                ...
                return new OkObjectResult($"Success " + localNow + ", "+ totalSent + " SMS sent");
            }
            catch (Exception ex)
            {
